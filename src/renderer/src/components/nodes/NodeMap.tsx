...
export function NodeMap({ onStartVerification }: NodeMapProps) {
  ...
  return (
    <div
      className="border-t flex-shrink-0 glass-panel-strong relative z-20"
      style={{
    ...
    >
      ...
      <div
        ...
        style={{ borderBottom: nodeMapCollapsed ? 'none' : '1px solid var(--border-subtle)' }}
      ...
      {!nodeMapCollapsed && (
        <div className="flex relative h-[calc(100%-36px)] overflow-hidden">
          ...
          <svg
            ...
            style={{ background: 'color-mix(in srgb, var(--bg-surface), transparent 40%)' }}
            ...
            {edges.map((e, i) => {
              ...
              return (
                <line
                  ...
                  stroke={isVerifiedEdge ? 'var(--node-verified)' : 'var(--border-default)'}
          ...
          {selectedNode && (selectedNode.state === 'unverified' || selectedNode.state === 'failed') && (
            <div
              className="w-72 border-l flex flex-col p-5 gap-4 z-20 transition-all duration-300 glass-panel shadow-2xl"
              style={{ borderColor: 'var(--border-subtle)' }}
            >
              <div className="flex items-center gap-2 border-b pb-2" style={{ borderColor: 'var(--border-subtle)' }}>
                <div className="w-1 h-4 rounded-full" style={{ background: 'var(--accent-primary)' }} />
              ...
              {selectedNode.dependencies.some(depId => activeNodes.find(n => n.id === depId)?.state !== 'verified') ? (
                ...
                <>
                  <p className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>
                    向小方解释「<span style={{ color: 'var(--accent-primary)' }}>{selectedNode.label}</span>」
                  </p>
                  <textarea
