...
export function AnswerInput({ onSubmit, onCancel, disabled, placeholder, compact = false }: AnswerInputProps) {
  ...
  return (
    <div className="relative">
          ...
          {showFormula && (
            <div className="mb-2">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button className="w-5 h-5 rounded flex items-center justify-center hover:opacity-80"
                  style={{ color: 'var(--text-muted)', background: 'rgba(0,0,0,0.2)' }} onClick={() => setShowFormula(false)}>
                  <X size={10} />
                </button>
              </div>
              <div className="flex flex-wrap gap-1 mb-1.5">
                {FORMULA_SNIPPETS.map((snippet) => (
                  <button key={snippet.id}
                    className="h-5 px-1.5 rounded text-[9px] border hover:opacity-80 transition-all"
                    style={{ borderColor: 'rgba(255,255,255,0.1)', color: 'var(--text-muted)', background: 'rgba(0,0,0,0.2)' }}
              ...
              <div className="flex gap-1">
                <input value={formulaDraft} onChange={(e) => setFormulaDraft(e.target.value)}
                  ...
                  placeholder="LaTeX..." />
                <button className="px-2 py-1 rounded text-[10px] font-medium transition-all hover:opacity-80"
          ...
          {showMarkdown && (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">
                ...
                <div className="h-6 px-1.5 rounded flex items-center gap-1 border" style={{ background: 'rgba(0,0,0,0.2)', borderColor: 'rgba(255,255,255,0.1)', color: 'var(--primary-color)' }}>
                  ...
                  <select value={markdownView} onChange={(e) => setMarkdownView(e.target.value as MarkdownView)}
                    className="terminal-select bg-transparent text-[9px] font-medium outline-none cursor-pointer" style={{ color: 'var(--primary-color)' }}>
      ...
      <div className="rounded-lg border transition-all relative" style={{
        borderColor: disabled ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.15)',
        background: 'rgba(0,0,0,0.2)',
      }}>
        ...
        {showMarkdown && (
          <div className="absolute top-1 right-1 z-10 flex items-center gap-1">
            <div className="relative group">
              ...
              {/* Dropdown */}
              <div className="absolute right-0 top-full mt-1 py-1 rounded-lg border shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 min-w-[120px]"
                style={{ borderColor: 'rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.9)' }}>
                <button
                  className="w-full px-3 py-1.5 text-left text-xs hover:opacity-80 flex items-center gap-2 transition-all"
        ...
        <div className="flex gap-1.5 p-1.5 items-end">
          ...
          {showMarkdown ? (
            <div className="flex-1 rounded border overflow-hidden" style={{
              borderColor: 'rgba(255,255,255,0.1)', minHeight: 60, maxHeight, background: 'rgba(0,0,0,0.15)'
            }}>
              {markdownView === 'split' ? (
                <div className="h-full flex">
                  <div className="flex-1 border-r" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
            ...
            <textarea ref={textareaRef}
              ...
              style={{ color: 'var(--text-primary)', minHeight: 44, maxHeight, borderColor: disabled ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.15)' }}
          ...
          <div className="flex items-center gap-1.5 pb-0.5">
            <div className="relative group">
              <button
                className={`w-7 h-7 rounded-lg transition-all flex items-center justify-center hover:opacity-80`}
                style={{ 
                  border: '1px solid rgba(255,255,255,0.1)',
                  background: showFormula ? 'rgba(0,0,0,0.3)' : 'transparent',
                  color: showFormula ? 'var(--primary-color)' : 'var(--text-muted)'
                }}
                title="公式"
              >
                <Sigma size={12} />
              ...
              {/* Formula Dropdown */}
              {showFormula && (
                <div className="absolute right-0 bottom-full mb-1 p-2 rounded-lg border shadow-lg z-20 min-w-[200px]">
                  <div className="flex flex-wrap gap-1 mb-1.5">
                    {FORMULA_SNIPPETS.map((snippet) => (
                      <button key={snippet.id}
                        className="h-5 px-1.5 rounded text-[9px] border hover:opacity-80 transition-all"
            ...
            {!showMarkdown && (
              <div className="relative group">
                ...
                <div className="absolute right-0 bottom-full mb-1 py-1 rounded-lg border shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 min-w-[100px]"
                  style={{ borderColor: 'rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.9)' }}>
                  <button
                    className="w-full px-3 py-1.5 text-left text-xs hover:opacity-80 transition-all"
                    style={{ color: 'var(--text-muted)' }}
            ...
            <button onClick={handleSubmit} disabled={disabledAction}
              ...
              style={{ background: disabledAction ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.5)', color: disabledAction ? 'var(--text-muted)' : 'var(--primary-color)', border: '1px solid rgba(255,255,255,0.1)' }}>
              <Send size={14} />
            </button>
          </div>
        </div>
