          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-[9px] opacity-50 px-1" style={{ color: 'var(--text-muted)' }}>未配置学生模型时，将自动回退使用专家模型。</p>
                    </div>

                    {/* Embedding Provider */}
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <label className="text-[11px] font-medium" style={{ color: 'var(--text-primary)' }}>知识库 Embedding 模型</label>
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-400 border border-green-500/20">知识检索 · 向量化</span>
                      </div>
                      <select
                        className="w-full rounded-lg px-2 py-2 text-xs outline-none transition-all duration-200 focus:ring-1 focus:ring-green-500/50"
                        style={{ background: 'var(--bg-base)', border: '1px solid var(--border-default)', color: 'var(--text-primary)' }}
                        value={settings.embeddingProviderId || ''}
                        onChange={(e) => {
                          const id = e.target.value
                          // 使用 setSettings 更新 embeddingProviderId
                          if (api) api.setSettings({ embeddingProviderId: id })
                          setSettings({ ...settings, embeddingProviderId: id })
                        }}
                      >
                        <option value="">-- 使用当前活动模型 --</option>
                        {settings.aiProviders.map(p => (
                          <option key={p.id} value={p.id}>{p.name} {p.embeddingModel ? `(${p.embeddingModel})` : ''}</option>
                        ))}
                      </select>
                      <p className="mt-1 text-[9px] opacity-50 px-1" style={{ color: 'var(--text-muted)' }}>
                        用于知识库文档向量化。如果当前模型不支持 Embedding，请配置一个支持的提供商（如 OpenAI）。
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <section>
              ...
              <div className="space-y-3">
                ...
                {/* Font Size */}
                <div className="flex items-center justify-between">
                  <label className="text-xs" style={{ color: 'var(--text-secondary)' }}>字体大小</label>
                  <div className="flex gap-1">
                    {(['sm', 'md', 'lg'] as const).map((size) => (
                      <button
                        ...
                        className="px-2 py-1 rounded text-xs"
                        style={{
                          background: settings.appearance.fontSize === size ? 'var(--accent-primary)' : 'var(--bg-elevated)',
                          color: settings.appearance.fontSize === size ? '#fff' : 'var(--text-muted)'
                        }}
                      >
                ...
                {/* Button Size */}
                <div className="flex items-center justify-between">
                  <label className="text-xs" style={{ color: 'var(--text-secondary)' }}>按钮大小</label>
                  <div className="flex gap-1">
                    {(['sm', 'md', 'lg'] as const).map((size) => (
                      <button
                        key={size}
                        onClick={() => updateAppearance({ buttonSize: size })}
                        className="px-2 py-1 rounded text-xs"
                        style={{
                          background: settings.appearance.buttonSize === size ? 'var(--accent-primary)' : 'var(--bg-elevated)',
                          color: settings.appearance.buttonSize === size ? '#fff' : 'var(--text-muted)'
                        }}
                      >
                        {size === 'sm' ? '小' : size === 'md' ? '中' : '大'}
                      </button>
                ...
                {/* Font Family */}
                <div className="flex items-center justify-between">
                  <label className="text-xs" style={{ color: 'var(--text-secondary)' }}>字体</label>
                  <select
                  ...
                  >
                    ...
                    <option value="'LXGW WenKai', 'PingFang SC', 'Microsoft YaHei'">霞鹜文楷</option>
          ...
          >
            ...
            <div className="space-y-2.5">
              {[
                ...
                { label: 'Embedding 模型', key: 'embeddingModel', placeholder: 'text-embedding-3-small (用于知识库)' },
