...
import { Minus, Square, X, BookOpen, Settings, MessageSquare, Keyboard, HelpCircle, ChevronLeft } from 'lucide-react'
...
export function TitleBar() {
  ...
  return (
    <div
      className="flex items-center h-[40px] select-none flex-shrink-0 header-bar"
      style={{
    ...
    >
      {/* Back button */}
      <button
        onClick={() => setActiveView(activeView === 'editor' ? 'welcome' : 'editor')}
        className="flex items-center justify-center w-14 h-full border-r transition-colors hover:bg-white/10"
        ...
        <button
          ...
          style={{
            background: activeView === 'editor'
              ? 'var(--accent-primary)'
              : 'transparent',
            color: activeView === 'editor' ? 'white' : 'var(--text-secondary)'
          }}
        >
          <BookOpen size={13} />
          编辑器
