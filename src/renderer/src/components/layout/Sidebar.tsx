...
function KnowledgeBaseSection({ notebookId }: { notebookId: string }) {
  ...
  return (
    <div className="mb-0.5">
      <div
        className="flex items-center gap-1 px-2 py-1 rounded cursor-pointer group transition-all duration-150 select-none"
      ...
      >
        ...
        <Database size={10} className="flex-shrink-0" style={{ color: 'var(--primary-color)' }} />
      ...
      {expanded && (
        <div className="ml-4">
          ...
          {files.length === 0 ? (
            <div
              className="text-[10px] px-2 py-2 italic flex items-center gap-1.5 cursor-pointer rounded transition-all duration-150"
              style={{ color: 'var(--text-muted)' }}
              onClick={handleUpload}
              onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
              onMouseLeave={(e) => e.currentTarget.style.background = ''}
            ...
            files.map((file) => (
              <div
                key={file.id}
                className="flex items-center gap-1.5 px-2 py-1 rounded group transition-all duration-150"
                style={{ color: 'var(--text-secondary)' }}
                onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
                onMouseLeave={(e) => e.currentTarget.style.background = ''}
              >
                <File size={10} style={{ color: 'var(--primary-color)', flexShrink: 0 }} />
...
function NoteItem({
  ...
  return (
    <div
      className={cn('flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer group transition-all duration-200')}
      style={{ background: isActive ? 'rgba(0,0,0,0.3)' : undefined }}
      onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.background = 'rgba(255,255,255,0.05)' }}
      onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.background = '' }}
      onClick={onClick}
    >
      <FileText size={12} style={{ color: isActive ? 'var(--primary-color)' : 'var(--text-muted)', flexShrink: 0 }} />
      <span
...
function NotebookItem({ notebook, notes }: { notebook: Notebook; notes: NoteMetadata[] }) {
  ...
  return (
    <div className="mb-1">
      <div
        className="flex items-center gap-1.5 px-2 py-1.5 rounded cursor-pointer group transition-all duration-200"
        onClick={() => setExpanded(!expanded)}
        style={{ color: 'var(--text-secondary)' }}
        onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
        onMouseLeave={(e) => e.currentTarget.style.background = ''}
      >
        ...
        <div
          ...
          style={{ background: notebook.color || 'var(--primary-color)' }}
        />
        ...
        <div className="ml-4 space-y-0.5">
          <div>
            ...
            >
              ...
              <FileText size={10} className="flex-shrink-0" style={{ color: 'var(--primary-color)' }} />
              <span className="text-[10px] font-medium flex-1" style={{ color: 'var(--text-muted)' }}>
              {'笔记'} ({notes.length})
              </span>
              <button
...
export function Sidebar() {
  ...
  return (
    <div
      ...
      style={{ width: 'var(--sidebar-width)', borderRight: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.15)' }}
    >
      ...
      {!isCpp && activeNodes.some(n => n.state === 'verified') && (
        <div className="px-2 pt-2 border-t" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
          <button
            onClick={() => {
            ...
            className="w-full h-12 rounded-xl flex items-center gap-3 px-3 transition-all duration-300 group hover:opacity-80"
            style={{
              background: 'rgba(0,0,0,0.2)',
              border: '1px solid rgba(255,200,0,0.2)'
            }}
          >
            <div className="w-8 h-8 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform" style={{ background: 'rgba(255,200,0,0.2)', color: '#ffc800' }}>
              <Brain size={16} />
            </div>
            <div className="text-left overflow-hidden">
              <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#ffc800' }}>记忆宫殿</div>
              <div className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>进入主动回忆环节</div>
            </div>
          </button>
        </div>
      )}
      ...
      <div className="p-2 border-t flex gap-1" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
        {isCpp ? (
          <button
            className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs transition-all duration-200 hover:opacity-80"
            style={{ background: 'rgba(0,0,0,0.2)', color: 'var(--primary-color)' }}
