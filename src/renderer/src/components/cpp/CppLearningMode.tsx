import React, { useState, useRef } from 'react'; // Re-saved to fix HMR
import { useStore } from '../../store';
import { useAI } from '../../hooks/useAI';
import type { CppFile } from '../../types/cpp-teaching.types';
import { 
  Send, 
  User, 
  Bot, 
...
import { CppVersionHistory } from './CppVersionHistory';
...
/**
 * CppLearningMode - C++ 费曼学习模式
 * 
 * 简洁双栏：
 * - 左侧：代码输入（完全空白，用户自己输入）
 * - 右侧：与小方对话
 */
export function CppLearningMode() {
  const { 
    cppFiles, 
    activeCppFileId, 
    updateCppFile,
  ...
  const activeFile = cppFiles.find((f: CppFile) => f.id === activeCppFileId);
    ...
    <div className="flex flex-col h-full bg-[#0d1117] text-gray-300">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-[#161b22] border-b border-gray-700/50">
        <div className="flex items-center gap-2 text-blue-400">
          <FileCode size={18} />
          <span className="font-semibold">C++ 费曼学习模式</span>
        </div>
          ...
          <div className="px-3 py-2 border-b border-gray-700/50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-base ${
                role === 'expert' ? 'bg-gradient-to-br from-yellow-400 to-amber-600 shadow-[0_0_10px_rgba(251,191,36,0.5)]' : 'bg-gradient-to-br from-blue-400 to-blue-600'
              }`}>
                {role === 'expert' ? '👨‍🏫' : '🧑‍🎓'}
              </div>
              <div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {showChatHistory ? (
              ...
              <div className="text-center text-gray-500 py-8">
                ...
                <p className="text-sm">向小方解释你的代码</p>
              </div>
            ) : (
              messages.map((msg) => (
                <StudentMessage
      ...
      {/* 底部 */}
      <div className="flex items-center justify-center px-4 py-1.5 bg-blue-600 text-white text-xs gap-2">
        <Terminal size={12} />
        <span>C++ 模式</span>
