...
function getActiveCreds(): { apiKey: string; baseURL: string; embeddingModel?: string; providerName?: string } | null {
...
async function extractText(filePath: string): Promise<string> {
  ...
  // Word documents
  if (ext === '.docx' || ext === '.doc') {
    // Note: Word document parsing requires additional dependencies
    // For now, we provide a helpful message. Users can convert to PDF/TXT/MD first.
    throw new Error('Word 文档暂不支持直接解析。请将文档转换为 PDF、TXT 或 Markdown 格式后上传。')
  }
  // PowerPoint documents
  if (ext === '.pptx' || ext === '.ppt') {
    throw new Error('PPT 文档暂不支持直接解析。请将演示文稿导出为 PDF 或 TXT 格式后上传。')
  }
  ...
  ipcMain.handle(
    ...
    async (_event, notebookId: string): Promise<IPCResponse<RagFileEntry>> => {
      try {
        ...
        const ext = extname(filePath).toLowerCase().replace('.', '') as 'pdf' | 'txt' | 'md' | 'docx' | 'doc' | 'pptx' | 'ppt'
        ...
        const chunkCount = await indexRawText(fileId, fileName, text, creds.apiKey, creds.baseURL, creds.embeddingModel)
  ...
  ipcMain.handle(
    ...
    async (_event, fileId: string): Promise<IPCResponse> => {
      try {
        const creds = getActiveCreds()
        if (creds) {
          await deleteFileFromIndex(fileId, creds.apiKey, creds.baseURL, creds.embeddingModel)
        }
