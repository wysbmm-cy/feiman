 ...
 */
export async function initRagService(apiKey: string, baseURL?: string): Promise<void> {
  if (_db && _currentApiKey === apiKey && _currentBaseURL === (baseURL ?? null)) return
...
export async function indexRawText(
  ...
  await initRagService(apiKey, baseURL)
...
export async function deleteFileFromIndex(fileId: string, apiKey: string, baseURL?: string): Promise<void> {
  await initRagService(apiKey, baseURL)
  if (!_db) return
 ...
 */
export async function indexNote(note: RagIndexNoteInput, apiKey: string, baseURL?: string): Promise<void> {
  await initRagService(apiKey, baseURL)
  if (!_db) return
...
export async function deleteNoteFromIndex(noteId: string, apiKey: string, baseURL?: string): Promise<void> {
  await initRagService(apiKey, baseURL)
  if (!_db) return
  const searchRes = await search(_db, {
    where: { noteId: noteId },
