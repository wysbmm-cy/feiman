...
#### 🆕 AI 代码纠错 (v1.1.0)
- **智能代码审查**：一键调用专家 AI 分析代码质量
- **差异对比显示**：红色高亮原代码问题，绿色高亮修正建议
- **修正理由注释**：每个修正都配有详细说明
- **一键应用修正**：快速采纳 AI 建议优化代码

#### 🎨 视觉更新 (v1.1.1)
- **费曼背景图**：新手引导界面添加理查德·费曼黑白背景图像，若隐若现的艺术效果
- **全新应用图标**：黑色背景配合橙色 "Af" 文字的现代简约风格
- **主题适配优化**：深色主题统一使用 #0d1117 背景，浅色主题下代码区使用白色背景
...
打包完成后，在 `dist/` 目录下会生成：
- `Anything Feynman Setup 1.1.1.exe` - NSIS 安装程序
- `Anything Feynman-1.1.1-win.exe` - 便携版（如果配置）
...
输出：
- `Anything Feynman-1.1.1.dmg` - DMG 安装镜像
- `Anything Feynman-1.1.1-mac.zip` - ZIP 压缩包
...
输出：
- `Anything Feynman-1.1.1.AppImage` - 通用 Linux 格式
- `Anything Feynman_1.1.1_amd64.deb` - Debian/Ubuntu 包
